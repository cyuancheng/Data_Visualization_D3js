...
p2 <- ggplot(data = data_cal_SF_a, aes(x=factor(month), y=remove_outliers(price))) +
      geom_boxplot()
...
list_df <- subset(data_SF_area, !is.na(Area) & !is.na(price.y),
                  select = c (neighbourhood, Area, latitude, longitude, room_type,
                minimum_nights, number_of_reviews, 
                availability_365, date, month, day, price.y))

data_SF_review <- subset(dlist_df,!is.na(Area)) %>%
            group_by(Area) %>%
            summarise(median_price =median(as.numeric(price.y)), 
                    median_aval =median(as.numeric(availability_365)/365*100) ,
            n=n())

data_SF_month <- subset(list_df, number_of_reviews >0 & !is.na(Area)) %>%
            group_by(room_type, Area, neighbourhood,  date, month, day) %>%
            summarise(median_price = median(as.numeric(price.y)), 
                    median_aval = median(as.numeric(availability_365)/365*100),
